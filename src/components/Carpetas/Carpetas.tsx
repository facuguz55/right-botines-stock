...
export function Carpetas({ modelos, onSell, onIngreso }: CarpetasProps) {
  ...
  const carpetas: Carpeta[] = useMemo(() => {
    ...
    for (const m of modelos) {
      ...
      if (!map.has(key)) {
        map.set(key, { categoria: m.categoria, gama: m.gama, modelos: [], totalPares: 0 })
      }
      const c = map.get(key)!
      c.modelos.push(m)
      c.totalPares += m.modelo_talles.reduce((s, t) => s + t.cantidad, 0)
    }
  ...
  if (carpetaAbierta) {
    const carpetaInfo = carpetas.find(
      c => c.categoria === carpetaAbierta.categoria && c.gama === carpetaAbierta.gama
    )
    return (
      <div className="carpetas-page">
        <div className="carpeta-header">
          ...
          <div className="carpeta-title-row">
            ...
            <span className="carpeta-count">{carpetaInfo?.modelos.length} modelos · {carpetaInfo?.totalPares} pares</span>
          </div>
        ...
        <div className="modelos-lista">
          ...
          {modelosEnCarpeta.map(m => {
            const totalPares = m.modelo_talles.reduce((s, t) => s + t.cantidad, 0)
                  ...
                  <div className="row-talles">
                    {m.modelo_talles.length === 0
                      ...
                      : m.modelo_talles.map(t => (
                          ...
                          >
                            {t.talle_arg}
                            {t.cantidad > 0 && <sup>{t.cantidad}</sup>}
                          </span>
                        ))
                    }
                  </div>
                </div>
                <div className="modelo-row-meta">
                  <span className="row-precio">${m.precio_venta.toLocaleString('es-AR')}</span>
                  <span className={`row-pares${agotado ? ' zero' : ''}`}>
                    {totalPares} {totalPares === 1 ? 'par' : 'pares'}
                  </span>
                </div>
                <div className="modelo-row-actions">
                  <button
                  ...
                  <button className="btn btn-secondary btn-sm" onClick={() => onIngreso(m)}>
                    + Stock
                  </button>
                </div>
              </div>
  ...
  return (
    <div className="carpetas-page">
      ...
      <div className="carpetas-grid">
        {carpetas.map(c => {
          const agotadas = c.totalPares === 0
          return (
            <button
              key={`${c.categoria}-${c.gama}`}
              className={`carpeta-card${agotadas ? ' vacia' : ''}`}
            ...
            >
              ...
              <div className="carpeta-stats">
                <span>{c.modelos.length} modelo{c.modelos.length !== 1 ? 's' : ''}</span>
                <span className={`carpeta-pares${agotadas ? ' zero' : ''}`}>
                  {c.totalPares} pares
                </span>
              </div>
            </button>
